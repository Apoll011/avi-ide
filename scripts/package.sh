...
if [ ! -e "src/api/api.h" ]; then
  echo "Please run this script from the root directory of Avi Studio."; exit 1
fi
...
main() {
  ...
  if [[ $source == true ]]; then source_package "avi-studio$version-src"; fi
  ...
  local exe_file="$(pwd)/${dest_dir}/avi-studio"

  local package_name=avi-studio$version-$platform-$arch
  local bundle=false
  ...
  if [[ -d "${data_dir}" ]]; then
    ...
    portable=true
    exe_file="$(pwd)/${dest_dir}/avi-studio"
    if [[ $platform == "windows" ]]; then
    ...
    if [[ -d "${data_dir}" ]]; then
      ...
      rm -rf "Avi Studio.app"; mv "${dest_dir}" "Avi Studio.app"
      dest_dir="Avi Studio.app"
      exe_file="$(pwd)/${dest_dir}/Contents/MacOS/avi-studio"
      data_dir="$(pwd)/${dest_dir}/Contents/Resources"
    fi
  fi

  if [[ $bundle == false && $portable == false ]]; then
    data_dir="$(pwd)/${dest_dir}/$prefix/share/avi-studio"
    exe_file="$(pwd)/${dest_dir}/$prefix/bin/avi-studio"
  fi
