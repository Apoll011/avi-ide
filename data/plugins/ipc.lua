...
---Run a single instance of Avi Studio.
...
config.plugins.ipc = common.merge({
  ...
  config_spec = {
    name = "Inter-process communication",
    {
      ...
      description = "Run a single instance of Avi Studio.",
...
---Constructor
---@param id? string Defaults to current Avi Studio process id.
function IPC:new(id)
  self.id = id or tostring(system.get_process_id())
  self.user_dir = USERDIR .. "/ipc"
  self.file = self.user_dir .. "/" .. self.id .. ".lua"
  self.shmem = package.loaded["shmem"] and shmem.open("avi-studio-ipc", 100) or nil
  self.primary = false
  self.running = false
  self.messages = {}
  self.replies = {}
...
---Get a list of running Avi Studio instances.
...
---Queue a new message to be sent to other Avi Studio instances.
---@param name string
...
---Get the IPC session for the running Avi Studio instance.
---@return plugins.ipc
...
system.show_fatal_error = function(title, message)
  if title == "Avi Studio internal error" then
    ipc:stop()
  end
